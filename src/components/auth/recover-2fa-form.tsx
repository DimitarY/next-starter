...
export function Recover2faForm({ className }: Recover2faFormProps) {
  ...
  const { mutate: Recover2faMutation, isPending: Recover2faMutationIsPending } =
    useMutation({
      mutationFn: async (values: z.infer<typeof Recover2faSchema>) => {
  ...
  const form = useForm<z.infer<typeof Recover2faSchema>>({
    resolver: zodResolver(Recover2faSchema),
    defaultValues: {
      backupCode: "",
    },
  });

  const onSubmit = async (values: z.infer<typeof Recover2faSchema>) => {
    Recover2faMutation(values);
  };
  ...
  return (
    <div className={cn("flex flex-col justify-center gap-1 p-4", className)}>
      <div className="space-y-2 text-left">
        ...
        <p>Enter one of your backup codes to recover your account.</p>
      </div>
      <Form {...form}>
                ...
                <FormItem>
                  ...
                  <FormControl>
                    <Input
                      ...
                      {...field}
                      disabled={Recover2faMutationIsPending}
                    />
                  </FormControl>
                  <FormMessage />
                </FormItem>
